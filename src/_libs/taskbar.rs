...
use winapi::um::winuser::{
    CreateWindowExA, DefWindowProcA, RegisterClassA, ShowWindow, WM_LBUTTONDOWN, WM_RBUTTONDOWN,
    WNDCLASSA,
};
...
pub unsafe fn create() -> HWND {
    ...
    let trayToolTip: String = "Pirate's Parrot".to_string();
    ...
    if hicon == std::ptr::null_mut() {
        println!("Error loading icon: {:?}", GetLastError());
    }
